...
def is_door(path, attribute):
	...
	return (2 < curve_length < 500) and (abs(test_ratio/ideal_ratio1 - 1) < 1e-1 or abs(test_ratio/ideal_ratio2 - 1) < 1e-1)
...
def remove_large_paths(paths,attributes,threshold):
	new_paths = []
	new_attributes = []
...
def remove_dots(paths, attributes, threshold):
	new_paths = []
	new_attributes = []
	wrong_paths = []
	wrong_attr = []
	...
	for path,attribute in zip(paths,attributes):
		...
		if real_path.length() < threshold:
			...
			wrong_attr.append(attribute)
			continue
		new_paths.append(path)
		new_attributes.append(attribute)

	return new_paths, new_attributes, wrong_paths, wrong_attr
...
def remove_empty_paths(paths, attributes):
	...
	for i, (path, attr) in enumerate(zip(paths, attributes)):
		...
		try:
			if 'rgb(100%,100%,100%)' in attr['style'].split(';')[0]:
				indices_to_remove.add(i)
		except KeyError:
			pass
...
def calc_segment_length(real_path):
	"""
	Returns length between endpoints of a given path
	"""
...
def determining_threshold_dots(svg_attributes):
	...
	img_length = 2*width + 2*height

	percentage_threshold = 0.5/(2*1224+2*792)

	return percentage_threshold*img_length


def deleteSVGLines(file, destination, threshold):
	print("Deleting the lines from " + file.split('/')[-1])
	...
	visualize_paths(paths, attributes, svg_attributes, destination)
